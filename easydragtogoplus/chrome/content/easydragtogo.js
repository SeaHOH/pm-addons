    ...
    openURL: function (aEvent,aURI, src, target, X, Y) {
        ...
        case "search-savetext":
          saveURL("data:text/plain," + "From URL:"+encodeURIComponent(gBrowser.currentURI.spec + "\r\n\r\n" + document.commandDispatcher.focusedWindow.getSelection()), gBrowser.selectedTab.label + ".txt",null, true, true, undefined,document);
            ...
            // The Original Code "search-list" from http://www.cnblogs.com/ziyunfei/archive/2011/12/20/2293928.html
            // has been modified by SeaHOH at 2018-11-15.
        case "search-list":
            try {
                ...
                var getSearchSubmission = this.getSearchSubmission;

                var serach = function (event) {
                    closeSerach();
                    var selectedEngine = event.target;
                    var act = "search-" + selectedEngine.label + "-fg";
                    var submission = getSearchSubmission(aURI, act);
                    if (submission) {
                        uri = submission.uri.spec;
                        gBrowser.loadOneTab(uri, aReferrerURI, null, submission.postData, false, false);
                    }
                    event.stopPropagation();
                };

                var closeSerach = function () {
                    this.removeEventListener("command", serach, true);
                    this.removeEventListener("popuphidden", closeSerach, false);
                    search_container.setAttribute("class", searchclass);
                };

                var searchPopup = document.getElementById("search-popup");
                searchPopup.addEventListener("command", serach, true);
                searchPopup.addEventListener("popuphidden", closeSerach, false);
                searchPopup.openPopup(null, null, easyDragToGo.onStartEvent.screenX - 100, easyDragToGo.onStartEvent.screenY - 100);
            } catch (e) {
